	...
	*/
	...
	function add_admin_notices() {
		...
		<div id="notice" class="update-nag">
			<?php _e(sprintf("%s is not configured, ", $this->plugin_name), $this->plugin_slug); ?>
			<a href="<?php menu_page_url( $this->plugin_slug, true ); ?>"><?php _e('Please configure it now!', $this->plugin_slug); ?></a>
		</div>
		<?php
	}
 	...
 	function add_setting_page_help(){
		...
		$help_content = 
			'<p>' . sprintf( __( 'Use this page to configure %s plugin', $this->plugin_slug ), $this->plugin_name) . '</p>' .
			'';
		...
		$screen->set_help_sidebar(
			...
			'<p><a href="https://esitedesign.com" target="_blank">' . __( 'Visit Our Website!',$this->plugin_slug ) . '</a></p>'
		);
	}
	...
	function register_settings() { // whitelist options
		register_setting(
			...
			add_settings_section(
				...
				function($args){ ?>
					<p id="<?php echo esc_attr( $args['id'] ); ?>"><?php _e('Please specify which libraries to load in your web site front-end', $this->plugin_slug); ?></p>
				<?php
				...
				add_settings_field(
					...
					function($args){
						...
						?>
						<input type="checkbox" name="besteditor[fontawesome]" id="fontawesome" value="true" <?php checked( true, $check_fontawesome ); ?> />
						<?php
				...
				add_settings_field(
					...
					function($args){
						...
						?>
						<input type="checkbox" name="besteditor[bootstrap]" id="bootstrap" value="true" <?php checked( true, $check_bootstrap ); ?> />
					<?php
					}, // Callback function to echo input tag
					$this->plugin_slug, // plugin slug, created by add_options_page()
					'load-libraries', // slug-name of the section
	...
	public function process_inputs( $input ){
		...
		if(false){
			add_settings_error(
				$this->plugin_slug . '_messages', // Slug title of setting
				'wporg_message', // Slug-name , Used as part of 'id' attribute in HTML output.
				__( 'Entered options are not valid!', $this->plugin_slug ), // message text, will be shown inside styled <div> and <p> tags
    ...
    function settings_page_content() {
		...
		<div class="wrap">
			...
			<p><?php _e('Best Editor automatically loads FontAwesome and Bootstrap 3.3.7 in editor area, But if you like to use Font Awesome and Bootstrap elements,<br />
								 You should load these libraries in your site frontend, you can enable them in following (if your theme doesn\'t load these libraries)', $this->plugin_slug); ?></p>
			<?php
				submit_button();
